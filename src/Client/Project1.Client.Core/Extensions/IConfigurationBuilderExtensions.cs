...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static IConfigurationBuilder AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var sharedAssembly = Assembly.Load("Project1.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("Project1.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"Project1.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "Project1.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("Project1.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"Project1.Client.Core.appsettings.{AppEnvironment.Current}.json");
